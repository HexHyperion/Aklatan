...
class BorrowRepository {
    ...
    suspend fun findActiveByBookId(bookId: Int): Borrow? = withTransaction {
        return@withTransaction BorrowEntity.find {
            (Borrows.book eq bookId) and
            Borrows.returnedAt.isNull()
        }
        .firstOrNull()
        ?.toBorrow()
    ...
    suspend fun updateEndsAt(bookId: Int, endsAt: Instant) = withTransaction {
        BorrowEntity.find {
            (Borrows.book eq bookId) and
            Borrows.returnedAt.isNull()
        }
    ...
    suspend fun updateReturnedAt(bookId: Int, returnedAt: Instant): Borrow? = withTransaction {
        BorrowEntity.find {
            (Borrows.book eq bookId) and
            Borrows.returnedAt.isNull()
        }
